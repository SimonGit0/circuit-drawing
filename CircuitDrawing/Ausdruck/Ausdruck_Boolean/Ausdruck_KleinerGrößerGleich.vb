Imports System.IO
Public Class Ausdruck_KleinerGrößerGleich
    ...
    Public Overrides Function AusrechnenSoweitMöglich() As Ausdruck_Boolean
        ...
        ElseIf TypeOf links Is Ausdruck_Variable AndAlso TypeOf rechts Is Ausdruck_Variable Then
            If DirectCast(links, Ausdruck_Variable).istGleich(DirectCast(rechts, Ausdruck_Variable)) Then
                Return New Ausdruck_Boolean_Konstante(True) 'Wenn beide Seiten gleich sind (a <= a, a >= a), dann ist es immer true
            End If
        End If
        Return Me
    End Function
    ...
    Public Shared Function load(reader As BinaryReader, version As Integer) As Ausdruck_KleinerGrößerGleich
        ...
        Return New Ausdruck_KleinerGrößerGleich(links, rechts, kleiner)
    End Function
